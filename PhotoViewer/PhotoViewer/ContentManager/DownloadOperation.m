...
-(void) start
{
    ...
    @autoreleasepool
    {
        ...
        NSMutableURLRequest *urlRequest = [[NSMutableURLRequest alloc] initWithURL:resourceURL cachePolicy:NSURLRequestUseProtocolCachePolicy timeoutInterval:120.0f];
        
        if(self.supportPauseResume)
        {
            _temporaryFile = [self createTemporaryDownloadFileForURL:resourceURL];
        ...
        NSString *mimeType = GetContentTypeForPathExtension([resourceURL pathExtension]);
        ...
        {
            NSLog(@"NSURLConnection can't handle request : %@", urlRequest);
            NSLog(@"URL : %@", urlRequest.URL);
            
            NSError *error = [NSError errorWithDomain:@"PhotoViewer" code:1004 userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Invalid resource url", NSLocalizedDescriptionKey, nil]];
...
-(void) deleteTemporaryDownloadFileForURL:(NSString*) partialDownloadFile
{
    ...
    if ([fileManager fileExistsAtPath:partialDownloadFile])
    {
        ...
        {
            NSLog(@"Unable to delete the partial downlaoded file : %@", error);
            assert(false);
        }
    }
}
...
-(void) restart
{
    ...
    if (error)
    {
        NSLog(@"Unable to delete the file : %@", error);
        assert(false);
    }
...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response
{
    ...
    NSLog(@"=== Mime Type from Response for [-- %@ --] is [-- %@ --]", self.request.resource.url, response.MIMEType);
    ...
    long long contentSize = (NSUInteger)[httpResponse.allHeaderFields objectForKey:@"Content-Length"] ? [[httpResponse.allHeaderFields objectForKey:@"Content-Length"] longLongValue] : [httpResponse expectedContentLength] > 0 ? [httpResponse expectedContentLength] : 0;
    ...
    {
        _expectedContentLength = (NSUInteger)contentSize;
    }
    ...
    if (self.supportPauseResume)
    {
        switch (statusCode)
        {
            case 206:
            {
                ...
                NSLog(@"Download Range Information : Byes Downloaded (%llu) Received Range (%llu)", downlaodedBytes, bytes);
        ...
        {
            NSLog(@"Failed to resume download for : %@",[_partialDownloadFile lastPathComponent]);
            [self restart];
        }
    }
    else
    {
        _responseData = [[NSMutableData alloc] initWithCapacity: (NSUInteger)contentSize];
    }
    if( statusCode / 100 != 2 )
    {
        NSLog(@"Download failed with error(%i) %@ for resource : %@",statusCode, [NSHTTPURLResponse localizedStringForStatusCode:statusCode],_resourceRequest.resource.url);
        if ([_resourceRequest.delegate respondsToSelector:@selector(downloadDidFail:withErro:)])
        {
            [_resourceRequest.delegate downloadDidFail:self withErro:[NSError errorWithDomain:@"PhotoViewer" code:statusCode userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSHTTPURLResponse localizedStringForStatusCode:statusCode], NSLocalizedDescriptionKey, nil]]];
        }
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data
{
    ...
    else
    {
        ...
        //NSLog(@"%@ : Bytes Received : %i, Current Progress : %i",[_resourceRequest.resource.url lastPathComponent], [data length], _progress);
...
{
    //log the error
     NSLog(@"Download failed with error : %@ for resource : %@",error,_resourceRequest);
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    if (self.supportPauseResume)
    {
        ...
        if (dataSize < _expectedContentLength)
        {
            [self deleteTemporaryDownloadFileForURL:_partialDownloadFile];
            NSLog(@"Error : file Download");
            _responseData = nil;
        }
        
        if (dataSize <= 0 || bytesDownloaded <= 0)
        {
            NSLog(@"Error : file Download");
        }
        assert(dataSize != 0);//Wht the file size is 0
        assert(bytesDownloaded != 0);//Wht the file size is 0
        self.supportPauseResume = NO;
    }
...
-(void)dealloc
{
    ...
    //void * info = [self observationInfo];
